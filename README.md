...
Several build flags are available for configuring the way the emulator works. The most important are `ONLINE_LINUX_IMAGE` and `ONLINE_LINUX`. If `ONLINE_LINUX_IMAGE` is set, the RAM image will be downloaded from my server or any other URL you specify in the code. If a WiFi connection can’t be made or the file can’t be retrieved, the badge automatically falls back to attempting to load from a 25Q32 SPI-Flash, which you must first wire up to the badge and configure the pinout in `lib/SPIFLASH/SPIFLASH.h`. Wire HOLD and WP to 3.3V. Not setting `ONLINE_LINUX_IMAGE` will cause it to always load from SPI-Flash. Do this when you’re not expecting to ever have a WiFi connection available and need to skip the connection attempts timing out.
